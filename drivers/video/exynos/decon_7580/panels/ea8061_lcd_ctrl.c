...
#ifdef CONFIG_LCD_DOZE_MODE
int aapav01_setalpm(struct dsim_device *dsim, int mode)
{
	int ret = 0;
...
static int aapav01_enteralpm(struct dsim_device *dsim)
{
	int ret = 0;
	struct panel_private *panel = &dsim->priv;
	...
	ret = aapav01_setalpm(dsim, panel->alpm_mode);
	if (ret < 0) {
...
static int aapav01_exitalpm(struct dsim_device *dsim)
{
	int ret = 0;
	struct panel_private *panel = &dsim->priv;
...
struct dsim_panel_ops ea8061_panel_ops = {
	...
	.init		= ea8061_init,
#ifdef CONFIG_LCD_DOZE_MODE
	.enteralpm = aapav01_enteralpm,
	.exitalpm = aapav01_exitalpm,
#endif
};
